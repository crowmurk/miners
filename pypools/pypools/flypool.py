...
class FlyPool(Pool):
    ...
    def __init__(self, account, coin=None, *, pool='api-zcash.flypool.org', port=443, timeout=5):
        """
            Аргументы:
        ...
        self.__queries = {'getMinerDashboard': 'dashboard', 'getMinerHistory': 'history',
                          'getMinerPayouts': 'payouts', 'getMinerSettings': 'settings',
                          'getMinerStatistics': 'currentStats', 'getWorkerAllStatistics': 'workers',
    ...
    def __getattr__(self, query):
        ...
        if query in self.__queries:
            def wrapper(*, worker=None, hours=None):
                # Формируем url
                url = '/miner/{account}/{query}'.format(account=self.__account, query=self.__queries[query])
                if worker:
                    url = url.format(worker=worker)
                elif '{worker}' in url:
                    raise TypeError("{function}() missing 1 required positional argument".format(function=query))
                # Отправляем запрос
