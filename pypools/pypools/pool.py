...
class Pool():
    ...
    def error(self, value):
        ...
        if isinstance(value, bool):
            ...
            raise ValueError(
                "pool request status '{error}'"
                " must be boolean type".format(
                    error=value,
                ),
            )
    ...
    @pool.setter
    def pool(self, value):
        """Адрес пула должен быть IP
        адресом или разрешенным DNS
        """
        try:
    ...
    def port(self, value):
        ...
        if value in range(1, 65536):
            ...
            raise ValueError(
                "pool port '{port}' must be in"
                " range 1..65535".format(
                    port=value,
                ),
            )
    ...
    def timeout(self, value):
        """Время ожидани ответа пула, должно быть
        в пределах от 1 до 60 и быть целым числом
        """
        if value in range(1, 61):
            self.__timeout = value
        else:
            raise ValueError(
                "pool response timeout '{timeout}'"
                " must be in range 1..60".format(
                    timeout=value,
                ),
            )
    ...
    def errorResponse(self, error, value):
        """Если не получен ответ от пула или
        неверный формат ответа заменяет ответ
        сообщением об ошибке и устанавливает
        соответвующий флаг
        """
        self.__error = True
    ...
    def sendRequest(self, url):
        ...
        try:
            ...
            # Ели ошибка, запускаем обработчик и
            # возвращаем соответсвующий статус
            try:
                self.errorResponse(
                    e, '{}: {} '.format(
                        result.status,
