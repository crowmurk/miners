...
class NanoPool(Pool):
    ...
    def __init__(
        ...
        self.__queries = {
            'getMinerAccountBalance': 'balance',
            'getWorkerCurrentHashrate': 'balance',
            'getMinerCheckMinerAccount': 'accountexist',
            'getMinerCurrentHashrate': 'hashrate',
            'getMinerGeneralInfo': 'user',
            'getMinerHashrateAndBalance': 'balance_hashrate',
            'getMinerListOfWorker': 'workers',
            'getMinerPayments': 'payments',
            'getMinerPaymentsDay': 'paymentsday',
            'getMinerWorkersLastReportedHashrate': 'reportedhashrates',
            'getUserSettings': 'usersettings',
            'getMinerAverageHashrates': 'avghashrate',
            'getMinerChartData': 'hashratechart',
            'getMinerHashrateHistory': 'history',
        ...
        self.__queriesHours = {
            'getMinerAverageHashrate': 'avghashratelimited',
        ...
        self.__queriesWorker = {
            'getWorkerAverageHashrates': 'avghashrate',
            'getWorkerChartData': 'hashratechart',
            'getWorkerHashrateHistory': 'history',
            'getWorkerLastReportedHashrateForWorker': 'reportedhashrate',
            'getWorkerShareRateHistory': 'shareratehistory',
            'getWorkerAverageHashrate': 'avghashratelimited',
        }
    ...
    def coin(self, value):
        ...
        if value in self.__coins:
            ...
            raise ValueError(
                "coin '{coin}' not supported by pool".format(
                    coin=value,
                ),
            )
        ...
        if query in self.queries:
            def wrapper(*, hours=None, worker=None):
                ...
                for item in (
                    ...
                    if query in item:
                        urlQuery = item[query]
                url = '/v1/{coin}/{query}/{account}'.format(
                    coin=self.__coin,
                ...
                if worker:
                    ...
                    if query in self.__queriesWorker:
                        ...
                        raise TypeError(
                            "{function}() got an unexpected"
                            " keyword argument 'worker'".format(
                                function=query,
                            ),
                        )
                ...
                if hours:
                    ...
                    if isinstance(hours, int) and hours in self.__hoursIntervals:
                        ...
                        if query in self.__queriesHours:
                            ...
                            raise TypeError(
                                "{function}() got an unexpected"
                                " keyword argument 'hours'".format(
                                    function=query,
                                ),
                            )
                    else:
                        raise ValueError(
                            "request period '{hours}' must"
                            " be in {intervals}".format(
                                hours=hours,
                                intervals=self.__hoursIntervals,
                            ),
                        )
