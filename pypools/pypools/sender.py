...
class Sender():
    ...
    def __init__(self, pools):
        """
           Аргументы:
               pools: список для опроса пулов в формате dict:
                   {PoolName: {'Pool': str, 'Coin': str, 'Host': str, 'Port': int, 'Timeout': int,
                               'Account': str, 'Request': dict, 'Workers': list, }, }
                   где:
                       Pool - тип пула
                       Coin - тип монеты
                       Host - адрес пула
        ...
        self.__supportedPools = {'NanoPool': NanoPool, 'FlyPool': FlyPool}
        self.__results = {}
        self.pools = pools
    ...
    def pools(self):
        """Список параметров опроса пулов в формате dict:
           {PoolName: {'Pool': str, 'Coin': str, 'Host': str, 'Port': int, 'Timeout': int,
                       'Account': str, 'Request': str, 'Workers': list, }, } """
        try:
    ...
    def pools(self, value):
        ...
        template = {
            "type": "object",
            "properties": {
                "Pool": {"type": "string"}, "Coin": {"type": "string"},
                "Host": {"type": "string"}, "Port": {"type": "integer"},
                "Timeout": {"type": "integer"}, "Account": {"type": "string"},
                "Workers": {"type": "array"},
        ...
        if not isinstance(value, dict):
            raise ValueError("pools list must be dictionary")
        ...
        for item in value:
            try:
                ...
                if value[item]['Pool'] not in self.supportedPools:
                    raise ValueError("Pool = '{pool}' not supported".format(pool=value[item]['Pool']))
            except ValueError as e:
                raise ValueError("pool = '{pool}' error in pool settings ({error})".format(pool=item, error=e)) from None
    ...
    @property
    def union(self):
        """Результаты опроса пулов, в формате dict с параметрами запросов и полученными ответами"""
        union = deepcopy(self.results)
        # Порядок ключей в возвращаемом словаре
        keys = ['Pool', 'Coin', 'Host', 'Port', 'Timeout', 'Account', 'Request', 'Workers', 'Result', 'Error']
    ...
    def sendRequests(self):
        ...
        for name, settings in self.pools.items():
            ...
            pool = self.__supportedPools[settings['Pool']](
                settings['Account'], settings['Coin'], pool=settings['Host'], port=settings['Port'], timeout=settings['Timeout'])
            # Отправляем запрос
            getattr(pool, settings['Request']['Name'])(**settings['Request']['Args'])
            # Сохраняем результат
            self.__results[name] = {'Result': pool.response, 'Error': pool.error}
